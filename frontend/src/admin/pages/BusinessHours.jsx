...
const inputCls = "bg-input border border-border text-foreground text-sm px-3 py-2 rounded-sm focus:outline-none focus:border-primary w-full"
const labelCls = "block text-muted-foreground text-xs uppercase tracking-wider mb-1"
  ...
  return (
    <div className="p-8 space-y-10">
      ...
      <div className="max-w-2xl">
        ...
        {blocks.length === 0 ? (
          <p className="text-muted-foreground text-sm">No hay fechas bloqueadas actualmente.</p>
        ) : (
            ...
            {blocks.map(b => (
              <div key={b.id}
                ...
                <div>
                  ...
                  {b.reason && (
                    <span className="ml-3 text-xs text-muted-foreground">— {b.reason}</span>
                  )}
                </div>
                <button onClick={() => handleDeleteBlock(b.id)}
                  className="text-xs text-muted-foreground hover:text-red-400 transition-colors ml-4 flex-shrink-0">
                  Eliminar
                </button>
              </div>
            ))}
